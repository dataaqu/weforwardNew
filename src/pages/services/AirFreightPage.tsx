...
import { Globe, Package, Thermometer, AlertTriangle } from 'lucide-react'
    ...
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-black' : 'bg-stone-100'}`}>
      ...
      <div className={`pt-20 pb-8 text-center ${
        ...
        <div style={{ marginTop: '30px' }}>
          <ServiceTextEffect serviceKey="airFreight" />
        </div>
      </div>
          ...
          <div className="flex justify-center">
            <div className={`p-4 md:p-8 rounded-lg border w-full ${
              ...
              <h3 lang={language} className="text-2xl md:text-3xl font-bold mb-6">
                {(() => {
                  if (language === 'ka') {
                    // Georgian: "საჰაერო გადაზიდვები"
                    return (
                      <>
                        <span className="text-white">საჰაერო</span>
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#309f69] to-[#2ff9c3]"> გადაზიდვები</span>
                      </>
                    )
                  } else {
                    // English: "AIR FREIGHT SERVICES"
                    return (
                      <>
              ...
              <div className="space-y-3 text-left">
                <div className={`text-base md:text-lg ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
                  {t.airFreight.services.description.split('\n').map((line, index) => {
                    const cleanLine = line.startsWith('•') ? line.substring(1).trim() : line
                    
                    const getIcon = (text: string) => {
                      if (text.includes('From any point') || text.includes('მსოფლიოს ნებისმიერი წერტილიდან')) {
                        return <Globe className="w-5 h-5 text-[#309f69] mr-2 flex-shrink-0 mt-1" />
                      }
                      if (text.includes('Dry general cargo') || text.includes('მშრალი ზოგადი ტვირთების')) {
                        return <Package className="w-5 h-5 text-[#309f69] mr-2 flex-shrink-0 mt-1" />
                      }
                      if (text.includes('Temperature-controlled') || text.includes('ტემპერატურით კონტროლირებადი')) {
                        return <Thermometer className="w-5 h-5 text-[#309f69] mr-2 flex-shrink-0 mt-1" />
                      }
                      if (text.includes('Dangerous cargo') || text.includes('სახიფათო ტვირთები')) {
                        return <AlertTriangle className="w-5 h-5 text-[#309f69] mr-2 flex-shrink-0 mt-1" />
                      }
                      return null
                    }
